...
class UNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.label_emb = nn.Embedding(LABEL_NUM, time_emb_dim)
    ...
    def forward(self, x, timestep, label):
        ...
        u1 = torch.cat(
            [u1, x2], dim=1
        )  # concat 64 + 128 = 192? No, x2 is 128, p1 is 64. Wait.
...
class DDPM(nn.Module):
    ...
    def forward(self, x_0, labels):
        ...
        # U-Net 预测噪声
        predicted_noise = self.model.forward(x_t, t, labels)
    ...
    def p_sample(self, x_t, t, t_index, labels):
        ...
        model_mean = sqrt_recip_alphas_t * (
            x_t - betas_t * self.model.forward(x_t, t, labels) / sqrt_one_minus_alphas_cumprod_t
        )
...
def generate_mode():
    ...
    target_digit = int(input("请输入想生成的数字 (0-9): "))
    labels = torch.full((1,), target_digit, device=DEVICE, dtype=torch.long)
    ...
    # 执行缓慢的逆向去噪采样
    num_samples = 1
    gen_imgs = ddpm.sample(num_samples, labels).cpu().numpy()
...
def denoise_process_mode():
    ...
    for i in reversed(range(0, ddpm.timesteps)):
        ...
        img = ddpm.p_sample(img, t, i, labels)
    ...
    fig, axes = plt.subplots(
        num_samples, len(stages), figsize=(len(stages) * 1.5, num_samples * 1.5)
    )
    ...
    for row in range(num_samples):
        for col, s in enumerate(stages):
            ax = axes[row, col]
            ax.imshow(
                stages_imgs[s][row].reshape(IMAGE_HEIGHT, IMAGE_WIDTH), cmap="gray"
            )
            ax.axis("off")
            if row == 0:
                ax.set_title(f"t={s}")
...
def evaluate_model():
    ...
    for x, _ in test_loader:
        ...
        collected += take
        if collected >= total_eval_samples:
            break
    ...
    try:
        ...
        logger.info("-" * 55)
        logger.info(
            f" 评估规模     : {total_eval_samples} 生成 vs {total_eval_samples} 真实"
        )
...
def main():
    while True:
        ...
        if choice == "1":
            ...
            while True:
                logger.info("\n  >>> 测试子菜单:")
                logger.info("  [1] Generate(按照指定的输入数字，生成对应的手写数字图片)")
                logger.info("  [2] Denoising Process (展示 5 张图逐渐去噪的帧序列)")
                logger.info(
                    "  [3] Forward & Backward (正向加噪-逆向生成全流程展示)"
                )  # 新增
