...
def diffusion_reconstruction_mode():
    """
    模式 2-3: MNIST 全流程可视化
    第一行: 原始数字 -> 逐步加噪 -> 纯噪声 (Forward Process)
    第二行: 纯噪声 -> 逐步去噪 -> 生成的新数字 (Reverse Process)
    """
    if not LATEST_MODEL_PATH.exists():
        logger.error("未找到模型，请先训练。")
        return

    logger.info("正在执行 MNIST [加噪-去噪] 全流程可视化...")
    ddpm = load_model()
    
    # 1. 从测试集中取出一个原始 MNIST 样本
    # 注意：MNIST 数据在加载时已经被 Normalize 到了 [-1, 1]
    it = iter(test_loader)
    x, _ = next(it)
    x_0 = x[0:1].to(DEVICE) # 取 Batch 中的第一张图 [1, 1, 28, 28]
    
    # 定义观察的时间节点 (0 代表原始图，1000 代表全噪声)
    stages = [0, 200, 400, 600, 800, 1000]
    
    # --- 过程一：正向加噪 (Forward Diffusion) ---
    forward_list = []
    for t_val in stages:
        if t_val == 0:
            forward_list.append(x_0)
        else:
            # 使用 DDPM 公式直接计算任意步数的加噪结果
    ...
    # --- 过程二：逆向去噪 (Reverse Reconstruction) ---
    # 我们从刚才加噪生成的最后一步噪声 (T=1000) 开始，尝试“还原”图片
    # 注意：生成的图片将与原图不同，但会是一个合法的 MNIST 数字
    backward_stages = {s: None for s in stages}
    curr_img = forward_list[-1] # T=1000 的纯噪声
    ...
    # --- 绘图可视化 ---
    num_cols = len(stages)
    fig, axes = plt.subplots(2, num_cols, figsize=(num_cols * 2, 4.5))
    
    for col in range(num_cols):
        # 第一行：展示加噪过程
        ax_f = axes[0, col]
        img_f = (forward_list[col][0].cpu() + 1) / 2 # [-1, 1] -> [0, 1] 用于显示
        ax_f.imshow(img_f.squeeze(), cmap='gray')
        ax_f.axis('off')
        ax_f.set_title(f"Fwd T={stages[col]}", fontsize=9)

        # 第二行：展示去噪过程 (顺序对应 stages 里的时间点)
        ax_b = axes[1, col]
        target_t = stages[col]
        img_b = (backward_stages[target_t][0].cpu() + 1) / 2
        ax_b.imshow(img_b.squeeze(), cmap='gray')
        ax_b.axis('off')
        ax_b.set_title(f"Rev T={target_t}", fontsize=9)

    plt.suptitle("MNIST DDPM: Forward Noise vs Reverse Reconstruction", fontsize=12)
    plt.tight_layout()
    
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    save_fn = IMAGE_DIR / f"mnist_full_process_{timestamp}.png"
    plt.savefig(save_fn, bbox_inches='tight')
    plt.close()
    logger.info(f"全流程可视化图已保存至: {save_fn.name}")
    ...
    while True:
        ...
        if choice == '1':
            ...
            while True:
                ...
                logger.info("  [3] Forward & Backward (正向加噪-逆向生成全流程展示)") # 新增
                ...
                if sub_choice == '1':
                    ...
                    diffusion_reconstruction_mode() # 调用新函数
