...
class ConditionEncoder(nn.Module):
    def __init__(self):
        ...
        super().__init__()
        # 这里需要确保输入的数字在0-20之间，大小才会是21
        self.n_embedding = nn.Embedding(21, 64)

        self.mu_mlp = nn.Sequential(nn.Linear(2, 64), nn.SiLU(), nn.Linear(64, 64))

        self.proj = nn.Sequential(nn.Linear(128, 256), nn.SiLU(), nn.Linear(256, 256))

    def forward(self, n, mu):
        n_emb = self.n_embedding(n)

        mu_rad = mu * math.pi / 180.0

        mu_feat = torch.stack([torch.sin(mu_rad), torch.cos(mu_rad)], dim=1)

        mu_emb = self.mu_mlp(mu_feat)

        cond = torch.cat([n_emb, mu_emb], dim=1)
...
class ConditionalUNet(nn.Module):
    ...
    def forward(self, noisy_images, timesteps, encoder_hidden_states):
        encoder_hidden_states = encoder_hidden_states.unsqueeze(1)
        return self.unet.forward(noisy_images, timesteps, encoder_hidden_states)
...
def train_model(train_loader, model: ConditionalUNet, optimizer, noise_scheduler):
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        for batch_idx, (images, class_id, mu) in enumerate(progress_bar):
            images = images.to(DEVICE)
            class_id = class_id.to(DEVICE)
            mu = mu.to(DEVICE)
            cond_emb = cond_encoder.forward(n=class_id, mu=mu)
            
            noise = torch.randn_like(images)
            batch_size = images.shape[0]
            timesteps = torch.randint(0, TIMESTEPS, (batch_size,), device=DEVICE).long()
            noisy_images = noise_scheduler.add_noise(images, noise, timesteps)
            noise_pred = model.forward(noisy_images, timesteps, cond_emb).sample
