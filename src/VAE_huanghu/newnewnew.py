# -*- coding: utf-8 -*-
"""
MNIST Conv-VAE (Improved for better N(0,I) sampling)
单文件可直接运行版本

核心修正：
1) LATENT_DIM = 8（降低潜空间难度）
2) BETA_MAX = 0.40（加强 posterior->prior 对齐）
3) 去掉 std floor（训练-推理一致）
4) beta 改为单调 warmup（不循环）
5) 保留 EMA、分类器评估、normal/agg 双采样、FID 评估
...
# =========================
@torch.no_grad()
def show_samples_standard_normal(model_eval, latent_dim, n=64, temperature=0.85, title="Generated MNIST (N(0,I))"):
    model_eval.eval()
    ...
    for i in range(n):
        ...
        plt.axis("off")
    plt.suptitle(f"{title}, T={temperature}")
    plt.tight_layout()
    plt.show()

@torch.no_grad()
def show_samples_agg_posterior(model_eval, train_loader, n=64, title="Generated MNIST (Aggregated Posterior)"):
    model_eval.eval()
    zs = []
    need = n
...
@torch.no_grad()
def check_latent_stats(model_eval, loader, max_batches=120):
    model_eval.eval()
    mus, stds = [], []
    cnt = 0
    for x, _ in loader:
    ...
    print(f"mean(|mu|): {mu_all.abs().mean().item():.4f}  (越接近0越好)")
    print(f"mean(std):  {std_all.mean().item():.4f}  (越接近1越好)")
    print(f"std(mu):    {mu_all.std().item():.4f}  (适中即可)")
...
@torch.no_grad()
def evaluate_generated_images(model_eval, clf, latent_dim, n_gen=10000, batch_size=256, mode="normal", ref_loader=None, temperature=1.0):
    model_eval.eval()
    clf.eval()
    ...
    while remain > 0:
        ...
        if mode == "normal":
            z = torch.randn(bs, latent_dim, device=device) * temperature
        elif mode == "agg":
            if ref_iter is None:
...
def build_resnet18_feature_extractor():
    try:
        ...
        mode = "random_init"
        print("[Warning] ResNet18 pretrained weights unavailable, fallback random.")
...
@torch.no_grad()
def evaluate_fid_resnet18(model_eval, test_loader, latent_dim, n_gen=10000, batch_size=256, mode="normal", ref_loader=None, temperature=1.0):
    model_eval.eval()
...
show_samples_standard_normal(eval_model, LATENT_DIM, n=64, temperature=0.85, title="Generated MNIST (N(0,I))")
show_samples_agg_posterior(eval_model, train_loader, n=64, title="Generated MNIST (Aggregated Posterior)")
...
check_latent_stats(eval_model, test_loader)
...
gen_conf_n, gen_entropy_n, gen_coverage_n = evaluate_generated_images(
    eval_model, clf, latent_dim=LATENT_DIM, n_gen=N_GEN_EVAL, batch_size=GEN_BATCH,
    mode="normal", temperature=1.0
)
...
fid_n, fid_mode_n = evaluate_fid_resnet18(
    eval_model, test_loader, latent_dim=LATENT_DIM, n_gen=N_GEN_EVAL, batch_size=GEN_BATCH,
    mode="normal", temperature=1.0
)

gen_conf_a, gen_entropy_a, gen_coverage_a = evaluate_generated_images(
    eval_model, clf, latent_dim=LATENT_DIM, n_gen=N_GEN_EVAL, batch_size=GEN_BATCH,
    mode="agg", ref_loader=train_loader
)
...
print("\n---- Sampling by Aggregated Posterior ----")
